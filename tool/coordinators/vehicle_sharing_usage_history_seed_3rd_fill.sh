#!/usr/bin/env bash
# Coordinator: seed-only usage history through 3rd full tank + Monica session.
# Louys: active share + journal through 3rd plein + 20 L + Monica session to
# 52 000.0 km; Monica: same fuels via catch-up (incl. fill-3) + that session.
...
SCENARIO_ID="${COMPARTARENTA_MULTI_SCENARIO_ID:-vehicle_sharing_usage_history_seed_3rd_fill}"
...
echo "================================================================================"
echo "Vehicle sharing USAGE HISTORY 3rd-fill seed"
...
echo "Manual: both AVDs seeded with usage-history journal (through 3rd plein + Monica session)."
exit 0
